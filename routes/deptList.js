...
app.post('/addDepartment', upload.single('picture'), (req, res) => {
    if(!req.session.profile)
        return res.redirect('/login')
    ...
    var {CTHSSV , PDH , PKT , PTC , CLC , TTTH , SDTC , ATEM , CSV , KL , TTNNTHVH , VCSKT , KDDT , KCNTT , LQTKD , KMT , KLDCD , KTCNH , KGDQT , KMTCN , PSDH , PDTMT} = req.body

    
    var arrayTemp = [CTHSSV , PDH , PKT , PTC , CLC , TTTH , SDTC , ATEM , CSV , KL , TTNNTHVH , VCSKT , KDDT , KCNTT , LQTKD , KMT , KLDCD , KTCNH , KGDQT , KMTCN , PSDH , PDTMT]
    var auths = []
    arrayTemp.forEach(val => {
        if(val)
            auths.push(parseInt(val))
    })
    
    var picture = req.file
    fs.renameSync(picture.path, `./public/images/${picture.originalname}`)
    if(idUser && name && email && picture){
        users.find({"idUser" : idUser}).exec()
        .then(data => {
            if(data.length)
                return res.redirect('/addDepartment?mess=add fail')
            ...
            NewUsers.save()
            return res.redirect('/addDepartment?mess=add success')
        })
